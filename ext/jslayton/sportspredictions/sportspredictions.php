...
namespace jslayton\sportspredictions;
...
class sportspredictions
{
	...
	public function __construct(\phpbb\auth\auth $auth, \phpbb\cache\service $cache, \phpbb\config\config $config, \phpbb\db\driver\driver_interface $db, \phpbb\template\template $template, \phpbb\user $user, \phpbb\controller\helper $helper, $root_path, $php_ext)
	{
		$this->auth = $auth;
		$this->cache = $cache;
		$this->config = $config;
		...
		*/
		...
		if (!$this->in_admin)
		{
			if ($user->data['user_sp_default_league'] != 0)
			{
				...
				{
					$this->league_id = $this->sp_config['default_league'];
					
					$data['user_sp_default_league'] = $this->sp_config['default_league'];
...
/**
* Fill $this->sp_config with cache if possible, if not load from DB
*
*/
	function load_config()
	{
		...
		$this->sp_config = $cache->get('_sp_config');
		
		if (empty($this->sp_config))
		{
			$sql = 'SELECT * FROM ' . SP_CONFIG_TABLE;
			$result = $db->sql_query($sql);
			while ($row = $db->sql_fetchrow($result))
			{
				$this->sp_config[$row['config_name']] = $row['config_value'];
			}
			$db->sql_freeresult($result);
			
			if ($this->sp_config['use_cache'] == 0)
			{
				$this->sp_config['cache_ttl'] = 0;
			}
			
			if ($this->sp_config['use_cache'])
			{
				$cache->put('_sp_config', $this->sp_config);
			}
		}
...
*/	
	function load_leagues_array()
	{
		...
		$result = $db->sql_query($sql, $this->sp_config['cache_ttl']);
		while ($row = $db->sql_fetchrow($result))
		{
	...
	function build_leaderboard($sort = 'points', $limit = false, $start = 0)
	{
		...
		if (empty($this->stats_array[$this->league_id]))
		{
			...
			*/
						...
						foreach ($prediction_array[$_game_id] AS $_user_id => $_prediction_data)
						{
							if ($this->leagues_array[$this->league_id]['scoring_style'] == 'score')
							{
								if (($_prediction_data['away_prediction'] == $_game_data['away_score']) && ($_prediction_data['home_prediction'] == $_game_data['home_score']))
								{
									...
									$_tmp_stats[$_user_id]['points'] += $this->sp_config['exact_prediction_points'];
									
									$_tmp_round_stats[$_user_id]['wins']++;
									$_tmp_round_stats[$_user_id]['points'] += $this->sp_config['exact_prediction_points'];
								...
								elseif ((($_prediction_data['away_prediction'] > $_prediction_data['home_prediction']) && ($_game_data['away_score'] > $_game_data['home_score'])) || (($_prediction_data['away_prediction'] < $_prediction_data['home_prediction']) && ($_game_data['away_score'] < $_game_data['home_score'])))
								{
									// correct winner, wrong score
									$_tmp_stats[$_user_id]['wins']++;
									$_tmp_stats[$_user_id]['points'] += $this->sp_config['correct_prediction_points'];
									...
									$_tmp_round_stats[$_user_id]['wins']++;
									$_tmp_round_stats[$_user_id]['points'] += $this->sp_config['correct_prediction_points'];
								...
								else
								{
									...
									$_tmp_stats[$_user_id]['points'] -= $this->sp_config['incorrect_prediction_points'];
									$_tmp_stats[$_user_id]['pointdiff'] += abs((intval($_prediction_data['away_prediction']) + intval($_prediction_data['home_prediction'])) - (intval($_game_data['away_score']) + intval($_game_data['home_score'])));
									
									$_tmp_round_stats[$_user_id]['losses']++;
									$_tmp_round_stats[$_user_id]['points'] -= $this->sp_config['incorrect_prediction_points'];
									$_tmp_round_stats[$_user_id]['pointdiff'] += abs((intval($_prediction_data['away_prediction']) + intval($_prediction_data['home_prediction'])) - (intval($_game_data['away_score']) + intval($_game_data['home_score'])));
							...
							else
							{
								if ($_prediction_data['away_prediction'] == $_prediction_data['home_prediction'] && $_game_data['away_score'] == $_game_data['home_score'])
								{
									// draw
									$_tmp_stats[$_user_id]['wins']++;
									$_tmp_stats[$_user_id]['points'] += $this->sp_config['draw_prediction_points'];

									$_tmp_round_stats[$_user_id]['wins']++;
									$_tmp_round_stats[$_user_id]['points'] += $this->sp_config['draw_prediction_points'];
			...
			if ($this->sp_config['use_cache'])
			{
				$cache->put('_sp_stats_array_' . $this->league_id, $this->stats_array[$this->league_id], $this->sp_config['cache_ttl']);
				$cache->put('_sp_stats_by_round_' . $this->league_id, $this->stats_by_round[$this->league_id], $this->sp_config['cache_ttl']);
			}
		}
...
*/
	...
	function upload_logo($form_name, $prefix)
	{
		...
		$upload = new fileupload('SP_', array('jpg', 'jpeg', 'gif', 'png'), false, 0, 0, $this->sp_config['logo_max_width'], $this->sp_config['logo_max_height']);
		if ($upload->is_valid($form_name))
		{
			$file = $upload->form_upload($form_name);
			$file->clean_filename('real', $prefix);
			$file->move_file($this->sp_config['logo_path'], true);
			@chmod($phpbb_root_path . $this->sp_config['logo_path'] . '/' . $file->realname, 0644);
			...
			else
			{
				...
				$thumbnail = $this->create_thumbnail($phpbb_root_path . $this->sp_config['logo_path'] . '/' . $file->realname);
				if ($thumbnail)
				{
...
*/
	...
	function create_thumbnail($source)
	{
		...
		if ($src_width > $this->sp_config['logo_max_thumbnail_width'] || $src_height > $this->sp_config['logo_max_thumbnail_height'])
		{
			...
			{								
				...
				if ($ratio < 1)
				{
					$dest_width = $this->sp_config['logo_max_thumbnail_height'] * $ratio;
					$dest_height = $this->sp_config['logo_max_thumbnail_height'];
				}
				else
				{
					$dest_width = $this->sp_config['logo_max_thumbnail_width'];
					$dest_height = $this->sp_config['logo_max_thumbnail_width'] / $ratio;
				}
				
				$_filename_ary = explode('.', basename($source));
				...
				switch($src_type)
				{
					case IMAGETYPE_JPEG:
						...
						{
							$_thumbnail_src = imagecreatefromjpeg($source);
						}
					break;
					...
					case IMAGETYPE_GIF:
						...
						{
							$_thumbnail_src = imagecreatefromgif($source);
						}
					break;
					...
					case IMAGETYPE_PNG:
						...
						{
							$_thumbnail_src = imagecreatefrompng($source);
						}
					break;
				}
				...
				if ($_thumbnail_src)
				{
					...
					{
						case IMAGETYPE_JPEG:
							imagejpeg($_thumbnail, $phpbb_root_path . $this->sp_config['logo_path'] . '/' . $thumbnail_filename, 90);
						break;

						case IMAGETYPE_GIF:
							imagegif($_thumbnail, $phpbb_root_path . $this->sp_config['logo_path'] . '/' . $thumbnail_filename);
						break;

						case IMAGETYPE_PNG:
							imagepng($_thumbnail, $phpbb_root_path . $this->sp_config['logo_path'] . '/' . $thumbnail_filename);
						break;
					}
					imagedestroy($_thumbnail);
									
					@chmod($phpbb_root_path . $this->sp_config['logo_path'] . '/' . $thumbnail_filename, 0644);
...
*/
		...
		switch ($category)
		{
			case 'league':
				...
				if (isset($this->leagues_array[$id]))
				{
					...
					{						
						if (file_exists($phpbb_root_path . $this->sp_config['logo_path'] . '/' . $this->leagues_array[$id]['league_logo']))
						{
							@unlink($phpbb_root_path . $this->sp_config['logo_path'] . '/' . $this->leagues_array[$id]['league_logo']);
						}
					}
						
					if (!empty($this->leagues_array[$id]['league_logo_tn']))
					{
						if (file_exists($phpbb_root_path . $this->sp_config['logo_path'] . '/' . $this->leagues_array[$id]['league_logo_tn']))
						{
							@unlink($phpbb_root_path . $this->sp_config['logo_path'] . '/' . $this->leagues_array[$id]['league_logo_tn']);
						}
					}
			...
			case 'team':
				...
				if (isset($this->teams_array['id_ref'][$id]))
				{
					...
					{
						if (file_exists($phpbb_root_path . $this->sp_config['logo_path'] . '/' . $this->teams_array['id_ref'][$id]['team_logo']))
						{
							@unlink($phpbb_root_path . $this->sp_config['logo_path'] . '/' . $this->teams_array['id_ref'][$id]['team_logo']);
						}
					}
						
					if (!empty($this->teams_array['id_ref'][$id]['team_logo_tn']))
					{
						if (file_exists($phpbb_root_path . $this->sp_config['logo_path'] . '/' . $this->teams_array['id_ref'][$id]['team_logo_tn']))
						{
							@unlink($phpbb_root_path . $this->sp_config['logo_path'] . '/' . $this->teams_array['id_ref'][$id]['team_logo_tn']);
						}
					}
	...
	function build_gametime_options($default_time)
	{
		...
		$default_time_ary = array(
			...
			'ampm'		=> gmdate('a', ($default_time + $timezone_offset))
		);
		...
		$ampm_opts = ($default_time_ary['ampm'] == 'pm') ? '<option value="0">--</option><option value="am">AM</option><option value="pm" selected="selected">PM</option>' : '<option value="0">--</option><option value="am" selected="selected">AM</option><option value="pm">PM</option>';
