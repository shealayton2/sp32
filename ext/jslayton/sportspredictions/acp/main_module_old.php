...
{	
	...
	public function main($id, $mode)
	{
		...
		global $language, $template, $config, $cache, $phpbb_container, $request, $user;
		...
		$language->add_lang(array('common'), 'jslayton/sportspredictions');
		...
		$this->logo_path = $phpbb_root_path . $sportspredictions->sp_config['logo_path'];
	...
	function overview($id, $mode)
	{
		...
		$league_id	= $request->variable('league_id', $sportspredictions->sp_config['default_league']);
		...
		$stats_array = $sportspredictions->build_leaderboard($sort, $sportspredictions->sp_config['leaderboard_limit'], $start);
		...
		/*
		if (sizeof($sportspredictions->stats_array[$league_id]) > $sportspredictions->sp_config['leaderboard_limit']) {
			$template->assign_vars(array(
				'PAGINATION'	=> generate_pagination($this->u_action . "&amp;sort=$sort", sizeof($sportspredictions->stats_array[$league_id]), $sportspredictions->sp_config['leaderboard_limit'], $start),
				'S_ON_PAGE'		=> on_page(sizeof($sportspredictions->stats_array[$league_id]), $sportspredictions->sp_config['leaderboard_limit'], $start)
			));
		}
		*/
		
		$pagination = $phpbb_container->get('pagination');
		$pagination->generate_template_pagination($this->u_action . "&amp;sort=$sort", 'paginated', 'page', sizeof($sportspredictions->stats_array[$league_id]), $sportspredictions->sp_config['leaderboard_limit'], $start);
		$template->assign_vars(array(
			'S_ON_PAGE' => $pagination->on_page(sizeof($sportspredictions->stats_array[$league_id]), $sportspredictions->sp_config['leaderboard_limit'], $start),
		));
	...
	function configuration($id, $mode)
	{
				...
				if (confirm_box(true))
				{
					foreach ((array) $sportspredictions->leagues_array AS $league_id => $league_info)
					{
						if ($league_info['league_logo'] != '')
						{
							...
							{
								if (file_exists($phpbb_root_path . $sportspredictions->sp_config['logo_path'] . '/' . $league_info['league_logo_tn']))
								{
									@unlink($phpbb_root_path . $sportspredictions->sp_config['logo_path'] . '/' . $league_info['league_logo_tn']);
								}
							}
								
							$thumbnail = $sportspredictions->create_thumbnail($phpbb_root_path . $sportspredictions->sp_config['logo_path'] . '/' . $league_info['league_logo']);
					...
					foreach ((array) $sportspredictions->teams_array['id_ref'] AS $team_id => $team_info)
					{
						if ($team_info['team_logo'] != '')
						{
							...
							{
								if (file_exists($phpbb_root_path . $sportspredictions->sp_config['logo_path'] . '/' . $team_info['team_logo_tn']))
								{
									@unlink($phpbb_root_path . $sportspredictions->sp_config['logo_path'] . '/' . $team_info['team_logo_tn']);
								}
							}
								
							$thumbnail = $sportspredictions->create_thumbnail($phpbb_root_path . $sportspredictions->sp_config['logo_path'] . '/' . $team_info['team_logo']);
		...
		$s_league_options = $sportspredictions->build_league_options($sportspredictions->sp_config['default_league']);
		
		$template->assign_vars(array(
			'S_LEAGUE_OPTIONS'				=> $s_league_options,

			'LEADERBOARD_LIMIT'				=> $sportspredictions->sp_config['leaderboard_limit'],
			'UPCOMING_GAMES_LIMIT'			=> $sportspredictions->sp_config['upcoming_games_limit'],
			'EXACT_PREDICTION_POINTS'		=> $sportspredictions->sp_config['exact_prediction_points'],
			'CORRECT_PREDICTION_POINTS'		=> $sportspredictions->sp_config['correct_prediction_points'],
			'INCORRECT_PREDICTION_POINTS'	=> $sportspredictions->sp_config['incorrect_prediction_points'],
			'LOGO_PATH'						=> $sportspredictions->sp_config['logo_path'],
			'LOGO_MAX_THUMBNAIL_WIDTH'		=> $sportspredictions->sp_config['logo_max_thumbnail_width'],
			'LOGO_MAX_THUMBNAIL_HEIGHT'		=> $sportspredictions->sp_config['logo_max_thumbnail_height'],
		...
		switch ($action)
		{
			...
			case 'save':
				...
				if ($league_id)
				{
					...
					if ($league_id == $sportspredictions->sp_config['default_league'] && $active == 0)
					{
						unset($sql_ary);
